...
var YJBJQuery = YJBJQuery || {};
(function ($) {
    //私有全局变量
    var url = 'YJBJ/YJBJHandler.ashx?req=';
    //结果面板容器ID
        ...
        var c = [
            ...
            { dataIndex: 'Num', text: '案件编号', flex: 1 },
            {
                dataIndex: 'ID', text: '操作', lock: true, sortable: false, width: 70, renderer: function (value, obj, record) {
                    ...
                    val = encodeURI(val);
                    var html = "<a href=\"#\" onclick=\"YJBJQuery.detailCallback('" + val + "')\" >详细</a>";
                    html += '&nbsp;&nbsp;&nbsp;&nbsp;';
                    html += "<a href=\"#\" onclick=\"YJBJQuery.locationCallback('" + val + "')\" >定位</a>";
                    return html;
                }
            }];
        return c;
    ...
    $.form = qForm.getQueryForm(qForm.qFormType.YJBJ, submitFn);
