 ...
 */
    Ext.define(name, {
        ...
        columns: [
            ...
            //{ text: '自定义文件名', flex:1, dataIndex: 'fileName', hidden:true, editor: { xtype: 'textfield' } },
            { text: '类型', width: 70, dataIndex: 'type' },
            {
                text: '大小(字节)', width: 100, dataIndex: 'size', renderer: function (v) {
            ...
            },
            {
                ...
                width: 50,
                items: [{
                    icon: 'Resources/images/extshare/delete.png',
                    tooltip: 'Remove',
        ...
        initComponent: function () {
            ...
            this.down('button[itemId=addFileBtn]').on({
                afterrender: function (btn) {
                    ...
                    this.swfupload.completecallback = this.completecallback || Ext.emptyFn;
...
})('Ext.ux.uploadPanel.UploadPanel', 'uploadpanel');

//Ext.form.field.VTypes
(function () {

    Ext.apply(Ext.form.VTypes, {
        confirmValue: function (val, field) {
            var defaults = {
                vtypeCallback: Ext.emptyFn,
                vtypemessage: undefined
            };
            Ext.apply(defaults, field);

            var me = this;

            function getText() {
                var msg = defaults.vtypemessage;

                if (msg && msg instanceof String)
                    return msg;

                if (msg && msg instanceof Function) {
                    return msg(val, field);
                }

                return null;
            }
            me.confirmValueText = getText() || me.confirmValueText;
            delete getText;

            var vl = defaults.vtypeCallback(val, field);
            if (vl === true)
                return true;
