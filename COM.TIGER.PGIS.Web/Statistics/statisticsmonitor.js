...
var $monitorstatistics = $monitorstatistics || {};
...
(function ($) {
    ...
    $.show = function () {
        $.loadStatistics(function () {
            ...
            var wind = ExtHelper.CreateWindow({ title: '监控数据统计 ...', width: 600, height: 400, layout: 'fit' });
            wind.add($.grid.Grid());
        });
    };

    $.showChart = function (options) {
        var wind = ExtHelper.CreateWindow({ title: '监控数据统计图表 ...', width: 600, height: 400, layout: 'fit' });
    ...
    function getChart(p, options) {
        var defaults = {
            ...
            xtitle: ['行政区划'],
            ytitle: ['数量'],
            url: String.Format('{0}monitor', basic_url),
...
})(Object.$Supper($monitorstatistics, 'model'));
...
(function ($) {
    ...
    var columns = [
        ...
        { dataIndex: 'Records', text: "监控数量", width: 75, sortable: false, hidden: false, renderer: rendercallback }
        //,
        //{
        //    dataIndex: 'ID', text: "", width: 65, sortable: false, hidden: false, renderer: function (a, b, c) {
        //        var data = c.getData();
        //        var val = Object.$EncodeObj(data);
        //        return String.Format('<span class="a" onclick="{0}(\'{1}\')">生成图表</span>', '$monitorstatistics.grid.ActionRender', val);
        //    }
        //}
    ];

    var Grid = $.Grid = function (options) {
        var defaults = { pager: false, columns: columns, req: 'monitor' };
        Ext.apply(defaults, options);
        ...
        var store = getStore({ url: String.Format('{0}{1}', $monitorstatistics.basic_url, defaults.req), total: defaults.pager });
    ...
    $.ActionRender = function (v) {
        ...
        var max = dat.Records;
        $monitorstatistics.showChart({ store: store, max: max + 100, vertical: false });
    };

    function getStore(options) {
        var defaults = { storeId: Ext.id(), url: $monitorstatistics.basic_url, model: $monitorstatistics.model.type, total: false, pageSize: 10 };
        Ext.apply(defaults, options);
...
})(Object.$Supper($monitorstatistics, "grid"));